{{-- resources/views/admin/pages/homepage/_news_card_fields.blade.php --}}
{{-- Expects $num (1 or 2) and $slug ('news') --}}
@php
    $prefix = "hp_news{$num}_"; // e.g., hp_news1_
    $image_path_key = $prefix . 'image'; // Key where the path is stored (e.g., hp_news1_image)
@endphp
    ...
    @if(!empty($settings[$image_path_key]))
        <div class="mb-2">
             <div class="inline-block p-1 border rounded-md bg-gray-50 align-top">
                ...
                <img src="{{ Storage::url($settings[$image_path_key]) }}?v={{ time() }}" alt="Current News {{ $num }} Image" class="h-16 max-w-xs block">
             </div>
