...
<section class="admin-content px-4 py-4 md:px-6 md:py-6">
    <div class="flex justify-between items-center mb-4">
        <h1 class="text-xl md:text-2xl font-semibold text-gray-700">Edit Menu Link: {{ $menuLink->title }}</h1>
    ...
    <div class="widget-card shadow-md border border-gray-100 rounded-lg overflow-hidden">
         ...
         <form action="{{ route('admin.menus.update', $menuLink) }}" method="POST" class="widget-content bg-white p-4 md:p-6 space-y-4">
            @csrf
            @method('PUT') {{-- Crucial for updates --}}

            {{-- Include the shared form fields --}}
            {{-- $menuLink and $locations variables are passed from MenuLinkController@edit --}}
            @include('admin.menus._form-fields', ['menuLink' => $menuLink, 'locations' => $locations ?? []])

            <div class="pt-3 text-right">
                <button type="submit" class="btn btn-primary shadow-md hover:shadow-lg">
                    <i class="fas fa-sync-alt fa-fw mr-1"></i> Update Menu Link
                </button>
            </div>
        </form>
    </div>
